...
$(document).ready(function() {
    ...
    function initSelect2() {
        $('#type-select2').select2({minimumInputLength: 0, language: 'ru'});
        $('#type-select2').on('select2:open', function() {
    ...
    function loadEditData(id, type_id, editUrl) {
        Swal.fire({ 
            title: 'Загрузка...', 
        ...
        $('#editModalLabel i').attr('class', 'mdi mdi-pencil-circle');
        
        $.ajax({
            url: editUrl + id + '?type_id=' + type_id,
            type: 'GET',
            success: (response) => {
                Swal.close();
                if (response.success) {
    ...
    $('#basic-datatable').on('click', '.edit-btn', function(e) {
        ...
        const type_id = $('#type-select2').val();
        loadEditData(id, type_id, editUrl);
    });
    ...
    $('#saveBtn').on('click', function() {
        ...
        if (!id) {
            // Новый параметр
            saveUrl = createUrl + '?type_id=' + type_id;
        } else {
