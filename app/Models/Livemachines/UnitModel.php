...
class UnitModel extends Model
{
    ...
    public function get_list($search = "", $start = 0, $length = 0, $orderColumn = 'name', $orderDir = 'asc') {
        ...
        if (!empty($search)) {
            ...
            $sqlWhereSearch = "AND `dirty_param_unit_value` LIKE {$searchTerm}";
        }
        ...
        $sql = "
            ...
            LEFT JOIN `dirty_param` ON (`dirty_param_unit_id` = `dirty_param_dirty_param_unit_id` AND `dirty_param_unit_dirty_type_id` = `dirty_param_dirty_type_id` AND `dirty_param_remove_user_id` = 0)
    ...
    public function get_id_from_name($name, $create = false) {
        ...
        try {
            ...
            if ($result) {
                $sql = "
                    UPDATE `dirty_param_unit`
                    SET
                        `dirty_param_unit_value` = ?
                    WHERE
                        `dirty_param_unit_id`    = ?
                ";
            ...
            // Создаем
                ...
                $sql = "
                    ...
                    SET
                        `dirty_param_unit_value`         = " . $this->pdo->quote((string)$name) . ",
                        `dirty_param_unit_dirty_type_id` = 1,
                        `dirty_param_unit_add_user_id`   = " . $this->pdo->quote((int)auth()->id()) . ",
                        `dirty_param_unit_add_date`      = UNIX_TIMESTAMP()
                ";
                $this->db->insert($sql);
    ...
    public function set($id, $name) {
        try {
            ...
            if (is_string($dbId) || $id == $dbId) {
                // Обновляем написание
                $this->db->update("UPDATE `dirty_param_unit` SET `dirty_param_unit_value` = ? WHERE `dirty_param_unit_id` = ?", [(string)$name, (int)$info->id]);
            } else {
                // Перепривязываем
                $this->db->update("UPDATE `dirty_param` SET `dirty_param_dirty_param_unit_id` = ? WHERE `dirty_param_dirty_param_unit_id` = ?", [(int)$dbId, (int)$info->id]);

                // Удаляем
                $this->db->update("
                    UPDATE `dirty_param_unit`
                    SET
                        `dirty_param_unit_remove_user_id` = ?,
                        `dirty_param_unit_remove_date`    = UNIX_TIMESTAMP()
                    WHERE
                        `dirty_param_unit_id` = ?
                    ",
                    [auth()->id(), (int)$id]
    ...
    public function remove($id) {
        try {
            ...
            $fileIds = $this->db->selectOne("SELECT GROUP_CONCAT(DISTINCT `dirty_param_dirty_file_id`) as `ids` FROM `dirty_param` WHERE `dirty_param_dirty_param_unit_id` = ? AND `dirty_param_remove_user_id` = 0", [(int)$id])->ids ?? "";
            ...
            $this->db->update("
                ...
                WHERE 1
                    AND `dirty_param_dirty_param_unit_id` = ?
