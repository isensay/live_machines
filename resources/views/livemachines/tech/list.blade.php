...
function createStickyHeader111() {
    ...
    // Создаем клон и добавляем его прямо перед оригиналом
    ...
    var originalLeft = $originalHeader.offset().left;
    
    // Настраиваем стили для фиксированного позиционирования
    $clone.css({
        'position': 'fixed',
        'top': '70px',
        ...
        'z-index': 9999,
        'background-color': $('.page-title-box').css('background-color'),
        'margin': '0',
        'border': 'none',
    });
    ...
    function updateSticky() {
        ...
        var currentBodyBg = $('.page-title-box').css('background-color');
        
        if (scrollTop > headerTop - 70) {
            // Обновляем позицию и ширину
            $clone.css({
                'left': $contentPage.offset().left + 'px',
                'background-color': currentBodyBg
            }).show();
    ...
    var observer = new MutationObserver(function() {
        if ($clone.is(':visible')) {
            $clone.css('background-color', $('.page-title-box').css('background-color'));
        }
    });
    observer.observe(document.body, { attributes: true, attributeFilter: ['class', 'data-theme'] });
}

function createStickyHeader222() {
    var $originalHeader = $('.page-title-box').first();
    var $window = $(window);
    var $contentPage = $('.content-page');
    var headerTop = $originalHeader.offset().top;
    ...
    function updateSticky() {
        ...
        if (scrollTop > headerTop - 70) {
            // При каждом показе берем свежий цвет из оригинала
            $clone.css({
                'left': $contentPage.offset().left + 'px',
                'background-color': $originalHeader.css('background-color')
            }).show();
...
function createStickyHeader333() {
    var $originalHeader = $('.page-title-box').first();
    var $window = $(window);
    var $contentPage = $('.content-page');
    var headerTop = $originalHeader.offset().top;
    ...
    $clone.css({
        ...
        'height': originalHeight + 'px',
        'z-index': 9999,
        'background-color': $originalHeader.css('background-color'), // Цвет из оригинала
        'margin': '0',
        'border': 'none',
    });
    ...
    // Простой способ отследить смену темы
    setInterval(function() {
        if ($clone.is(':visible')) {
            var currentColor = $originalHeader.css('background-color');
            var cloneColor = $clone.css('background-color');
            
            if (currentColor !== cloneColor) {
                $clone.css('background-color', currentColor);
            }
        }
    }, 100); // Проверяем каждые 100мс
}

function createStickyHeader444() {
    var $originalHeader = $('.page-title-box').first();
    var $window = $(window);
    var $contentPage = $('.content-page');
    var headerTop = $originalHeader.offset().top;
    ...
    function updateSticky() {
        ...
        if (scrollTop > headerTop - 70) {
            // СНАЧАЛА обновляем цвет из оригинала
            var currentColor = $originalHeader.css('background-color');
            $clone.css('background-color', currentColor);
            
            // ПОТОМ обновляем позицию и показываем
            $clone.css({
                'left': $contentPage.offset().left + 'px'
            }).show();
    ...
    // Отслеживаем смену темы (более надежно)
    var observer = new MutationObserver(function() {
        if ($clone.is(':visible')) {
            // При смене темы сразу обновляем цвет
            $clone.css('background-color', $originalHeader.css('background-color'));
        }
    });
    ...
    // Убираем setInterval, он больше не нужен
}
